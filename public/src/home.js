function getTotalBooksCount(books) 
{
  // YOUR SOLUTION HERE
  // return books.length;
  // Will try to use the .reduce() method to
  // pass a requirement of this assesment.
  let length = 0;
  length = books.reduce((accumulator, book) => 
  {
...
// Creating this helper function to fulfill a requirement
function getBooksBorrowedCount(books) 
{
  // YOUR SOLUTION HERE
  const bookList = books.filter((book) => !book.borrows[0].returned);
  let count = 0;
  function bookCounter(bookList)
  {
  ...
  //I know I could have done bookList.length in the return value.
  //But wanted to use the above code to fulfill a requirement.
  return bookCounter(bookList);
